...
2. 自动注入不支持方法参数注入， 目前只支持类属性注入。

3. 模块依赖的时候， 如果被依赖的模块有更新， 当前模块不能自动更新， 前面也提到过了。
...
### 3. 开发环境开启热更新
> 正常情况下 webpack环境下的热更新， 会依据更改的文件内容，依次往上找依赖的文件，直到调用过 module.hot.accept() 方法的模块，执行整体更新。 如果我们开发的过程中，
> 只修改Model文件， 没必要同时更新依赖Model的页面组件， 我们可以在Model文件底部加入module.hot && module.hot.accept(); 即可，达到最优开发体验；redux-spring 有对热更新做了良好的适配， 热更新数据不会丢失。
