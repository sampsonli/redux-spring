            ...
            if (key !== 'constructor' && typeof Clazz.prototype[key] === 'function') {
                ...
                prototype[key] = function () {
                    ...
                    if (isGen_1) { // 如果当前方法是generator方法
                        ...
                        // 异步方法必须异步执行
                        var result_1 = Promise.resolve().then(function () { return runGen_1(origin_1.bind(_this).apply(void 0, params), null, false, null); });
                        // @ts-ignore 此处为了给外部调用异步方法提供generate方法api支持， 方便项目使用ts接收返回值， 避免强制类型转换使用
                        result_1.return = result_1.next = result_1.then;
                        // @ts-ignore 此处为了给外部调用异步方法提供generate方法api支持， 方便项目使用ts接收返回值， 避免强制类型转换使用
                        result_1.throw = result_1.catch;
                        return result_1;
                    }
                    var rootState = _store.getState();
                    var state = rootState[ns];
